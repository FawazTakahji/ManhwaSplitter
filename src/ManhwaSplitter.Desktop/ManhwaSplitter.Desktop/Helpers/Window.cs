...
using ManhwaSplitter.Desktop.Models;
using NLog;
...
public static class Window
{
    ...
    public static void SaveLocation(this AvaloniaWindow window)
    {
        ...
        string filePath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "data", "window.json");
        ...
        try
        {
            string json = JsonSerializer.Serialize(windowLocation, Singletons.JsonSerializerOptions);
    ...
    private static WindowLocation? LoadLocation(string path)
    {
        ...
        try
        {
            ...
            return JsonSerializer.Deserialize<WindowLocation>(json, Singletons.JsonSerializerOptions);
        }
        catch (Exception ex)
        {
